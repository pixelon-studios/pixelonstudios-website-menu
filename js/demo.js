 ...
 */
{
    ...
    class MenuItem {
        ...
        toggle(action = 'show') {
            return new Promise((resolve, reject) => {
                const tx = action === 'hide' ? this.isCurrent() ? '-500%' : '100%' : this.isCurrent() ? '-100%' : '0%';
    ...
    class Menu {
        constructor(el) {
            ...
            // // MenuItem instances
            this.menuItems = [];
            [...this.DOM.menu.querySelectorAll('.menu__item')].forEach((item, position) => this.menuItems.push(new MenuItem(item)));
